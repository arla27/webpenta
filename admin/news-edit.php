...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
	    ...
	    if($previous_photo == '' && $path == '') {
	    	$statement = $pdo->prepare("UPDATE tbl_news SET news_title=?, news_slug=?, news_content=?, news_content_short=?, news_date=?, category_id=?,file_id=?, publisher=?, meta_title=?, meta_keyword=?, meta_description=? WHERE news_id=?");
	    	$statement->execute(array($_POST['news_title'],$news_slug,$_POST['news_content'],$_POST['news_content_short'],$_POST['news_date'],$_POST['category_id'],$_POST['file_id'],$publisher,$_POST['meta_title'],$_POST['meta_keyword'],$_POST['meta_description'],$_REQUEST['id']));
	    }
			...
			<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
				            <label for="" class="col-sm-3 control-label">File download </label>
				            <div class="col-sm-3">
				            	...
				            	foreach ($result as $row) {
									?>
									<option value="<?php echo $row['file_id']; ?>" <?php if($row['file_id']==$category_id){echo 'selected';} ?>><?php echo $row['file_title']; ?><?php echo " || "; ?><?php echo $row['file_name']; ?></option>
	                                <?php
								}
								?>
								</select>
