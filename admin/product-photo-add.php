...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
		...
		$final_name = 'product-photo-'.$ai_id.'.'.$ext;
        move_uploaded_file( $path_tmp, '../assets/uploads/product/'.$final_name );

		// saving into the database
		$statement = $pdo->prepare("INSERT INTO tbl_photo_product (p_product_caption,photo_name,p_name_id) VALUES (?,?,?)");
		$statement->execute(array($_POST['p_product_caption'],$final_name,$_POST['p_name_id']));

    	$success_message = 'Product photo is added successfully.';
    }
}
?>
				...
				<div class="box box-info">
					<div class="box-body">
						<div class="form-group">
							...
							<div class="col-sm-4">
								<input type="text" class="form-control" name="p_product_caption">
							</div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<select class="form-control" name="p_name_id">
									...
									foreach ($result as $row) {
										echo '<option value="'.$row['product_id'].'">'.$row['product_caption'].'</option>';
									}
									?>
								</select>
							</div>
