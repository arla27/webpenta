...
if(isset($_POST['form1'])) {
    ...
    if($path!='') {
    	...
    	$valid = 0;
        $error_message .= 'You must have to select a photo for store member photo<br>';
    }
	...
	if($valid == 1) {
		...
		$statement = $pdo->prepare("INSERT INTO tbl_team_member (name,slug,designation_id,photo,banner,degree,detail,facebook,twitter,linkedin,youtube,google_plus,instagram,address,practice_location,phone,email,website,status,meta_title,meta_keyword,meta_description) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
		...
		$success_message = 'Store Member is added successfully!';
