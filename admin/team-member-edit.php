...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['designation_id'])) {
		$valid = 0;
		$error_message .= 'You must have to select a branch<br>';
	}
    ...
    if($path!='') {
        ...
        if( $ext!='jpg' && $ext!='png' && $ext!='jpeg' && $ext!='gif' ) {
            $valid = 0;
            $error_message .= 'You must have to upload jpg, jpeg, gif or png file for Store Member photo<br>';
        }
    }
	...
	if($valid == 1) {
		...
		if($path == '' && $path1 == '') {
			$statement = $pdo->prepare("UPDATE tbl_team_member SET name=?, slug=?, designation_id=?, degree=?, detail=?, facebook=?, twitter=?, linkedin=?, youtube=?, google_plus=?, instagram=?, address=?, practice_location=?, phone=?, email=?, website=?, status=?, meta_title=?, meta_keyword=?, meta_description=? WHERE id=?");
    		$statement->execute(array($_POST['name'],$slug,$_POST['designation_id'],$_POST['degree'],$_POST['detail'],$_POST['facebook'],$_POST['twitter'],$_POST['linkedin'],$_POST['youtube'],$_POST['google_plus'],$_POST['instagram'],$_POST['address'],$_POST['practice_location'],$_POST['phone'],$_POST['email'],$_POST['website'],$_POST['status'],$_POST['meta_title'],$_POST['meta_keyword'],$_POST['meta_description'],$_REQUEST['id']));
		}
		...
		if($path == '' && $path1 != '') {
			...
			$statement = $pdo->prepare("UPDATE tbl_team_member SET name=?, slug=?, designation_id=?, banner=?, degree=?, detail=?, facebook=?, twitter=?, linkedin=?, youtube=?, google_plus=?, instagram=?, address=?, practice_location=?, phone=?, email=?, website=?, status=?, meta_title=?, meta_keyword=?, meta_description=? WHERE id=?");
		...
		if($path != '' && $path1 != '') {
			...
			$statement = $pdo->prepare("UPDATE tbl_team_member SET name=?, slug=?, designation_id=?, photo=?, banner=?, degree=?, detail=?, facebook=?, twitter=?, linkedin=?, youtube=?, google_plus=?, instagram=?, address=?, practice_location=?, phone=?, email=?, website=?, status=?, meta_title=?, meta_keyword=?, meta_description=? WHERE id=?");
    		$statement->execute(array($_POST['name'],$slug,$_POST['designation_id'],$final_name,$final_name1,$_POST['degree'],$_POST['detail'],$_POST['facebook'],$_POST['twitter'],$_POST['linkedin'],$_POST['youtube'],$_POST['google_plus'],$_POST['instagram'],$_POST['address'],$_POST['practice_location'],$_POST['phone'],$_POST['email'],$_POST['website'],$_POST['status'],$_POST['meta_title'],$_POST['meta_keyword'],$_POST['meta_description'],$_REQUEST['id']));
		}

	    $success_message = 'Store Member is updated successfully!';
	}
}
?>
			...
			<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
				...
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Person </label>
							<div class="col-sm-6">
						...
						<div class="form-group">
							<label for="" class="col-sm-2 control-label"><span>*iframe code</span> Map Location</label>
							<div class="col-sm-6">
