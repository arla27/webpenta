...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['p_category_name'])) {
    	...
    	$statement = $pdo->prepare("SELECT * FROM tbl_category_product WHERE p_category_name=?");
    ...
    if($valid == 1) {
		...
		$statement = $pdo->prepare("INSERT INTO tbl_category_product (p_category_name,status) VALUES (?,?)");
		$statement->execute(array($_POST['p_category_name'],$_POST['status']));

    	$success_message = 'Product Category is added successfully.';
    }
}
?>
