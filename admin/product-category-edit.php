...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['p_category_name'])) {
        $valid = 0;
        $error_message .= "Product Category Name can not be empty<br>";
    } else {
		...
		$statement = $pdo->prepare("SELECT * FROM tbl_category_product WHERE p_category_name=? and p_category_name!=?");
    	$statement->execute(array($_POST['p_category_name'],$current_category_name));
    	$total = $statement->rowCount();							
    	if($total) {
    		$valid = 0;
        	$error_message .= 'Product Category name already exists<br>';
    	}
    }
    ...
    if($valid == 1) {
		...
		$statement = $pdo->prepare("UPDATE tbl_category_product SET p_category_name=?, status=? WHERE p_category_id=?");
		$statement->execute(array($_POST['p_category_name'],$_POST['status'],$_REQUEST['id']));

    	$success_message = 'Product Category is updated successfully.';
    }
}
?>
...
<section class="content-header">
	...
	<div class="content-header-right">
		<a href="product-category.php" class="btn btn-primary btn-sm">View All</a>
	</div>
</section>
